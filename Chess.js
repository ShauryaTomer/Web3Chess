...
function connectWallet() {
  ...
  const accountAddress = accountInput.value;
  
  if (!accountAddress) {
    console.error('Please enter a valid account address');
    return;
  }
  ...
  if ('solana' in window) {
    window.solana.connect()
      .then(() => {
        ...
        alert("Connected");
        const publicKey = new solana.PublicKey(accountAddress);
        console.log('Public Key:', publicKey.toBase58());
        // Code to perform actions with the wallet goes here
      })
      .catch((error) => {
        console.error('Failed to connect to Solana wallet:', error);
      });
  } else {
    console.error('Solana Phantom extension is not installed');
  }
}
